...
import { setGhostLook, setMudLook } from '../src/game/vehicle-art.js';
...
/* ---- the triangle census -------------------------------------------------
   Counted once, because none of it changes after the build: the model is
   merged per material and the only thing that moves is a transform. Grouped
   BY MATERIAL rather than by mesh, because that is the axis a budget is
   actually spent along — "the tyres are 2136" is an answer you can act on,
   "mesh 14 is 534" is not. */
const census = (() => {
  const by = new Map();
...
function frame(now) {
  ...
  stats.textContent =
    ...
    `${spec.team || ''}\n` +
    `livery ${livery}   sky ${skyId}   ${fps.toFixed(0)} fps\n` +
